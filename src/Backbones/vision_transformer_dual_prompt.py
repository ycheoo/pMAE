...
from Backbones.prompt import EPrompt
...
class VisionTransformer(nn.Module):
    ...
    def __init__(
        ...
        if (
            not self.use_prefix_tune_for_g_prompt
...
# add sup
@register_model
def vit_base_patch16_224_sup_dualprompt(pretrained=False, **kwargs):
...
# add ibot
@register_model
def vit_base_patch16_224_ibot_dualprompt(pretrained=False, **kwargs):
    print("use ibot")
    model_kwargs = dict(patch_size=16, embed_dim=768, depth=12, num_heads=12, **kwargs)
    model = _create_vision_transformer(
