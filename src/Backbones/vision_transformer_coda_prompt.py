...
class VisionTransformer(nn.Module):
    ...
    def forward(
        self, x, register_blk=-1, prompt=None, q=None, train=False, task_id=None
    ):
        B = x.shape[0]
...
# add sup
@register_model
def vit_base_patch16_224_sup_coda_prompt(pretrained=False, **kwargs):
...
# add ibot
@register_model
def vit_base_patch16_224_ibot_coda_prompt(pretrained=False, **kwargs):
    print("use ibot")
    model_kwargs = dict(patch_size=16, embed_dim=768, depth=12, num_heads=12, **kwargs)
    model = _create_vision_transformer(
