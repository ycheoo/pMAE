...
class CodaPrompt(nn.Module):
    def __init__(self, emb_d, n_task, prompt_param, key_dim=768):
        super().__init__()
        self.emb_d = emb_d
        self.key_d = key_dim
        self.n_task = n_task
    ...
    def set_prompt(self, cur_task, known_tasks):
        self.cur_task = cur_task
        self.known_tasks = known_tasks

    def process_gram_schmidt(self):
    ...
    def gram_schmidt(self, vv):
        ...
        # print("Cur Task:", self.cur_task)
        # print("Known Tasks:", self.known_tasks)
        # get starting point
        pt = int(self.e_pool_size / (self.n_task))
        s = int(self.cur_task * pt)
        f = int((self.cur_task + 1) * pt)
        old_prompts = [
            list(range(start * pt, (start + 1) * pt)) for start in self.known_tasks
        ]
        old_prompts_idx = [idx for old_prompt in old_prompts for idx in old_prompt]
        if len(self.known_tasks) > 0:
            uu[:, old_prompts_idx] = vv[:, old_prompts_idx].clone()
        for k in range(s, f):
            redo = True
            while redo:
                redo = False
                ...
                uk = 0
                for j in old_prompts_idx + list(range(s, k)):
                    if not redo:
                        uj = uu[:, j].clone()
    ...
    def forward(self, x_querry, l, x_block, train=False):
        ...
        if l in self.e_layers:
            ...
            # print("All Tasks:", self.known_tasks)
            if train:
                s = int(self.cur_task * pt)
                f = int((self.cur_task + 1) * pt)
                freeze_tasks = [
                    task for task in self.known_tasks if task != self.cur_task
                ]
                old_prompts = [
                    list(range(start * pt, (start + 1) * pt)) for start in freeze_tasks
                ]
                old_prompts_idx = [
                    idx for old_prompt in old_prompts for idx in old_prompt
                ]
                # print("Training:", self.cur_task)
                # print("Freeze:", freeze_tasks)
                if len(freeze_tasks) > 0:
                    K = torch.cat((K[old_prompts_idx].detach().clone(), K[s:f]), dim=0)
                    A = torch.cat((A[old_prompts_idx].detach().clone(), A[s:f]), dim=0)
                    p = torch.cat((p[old_prompts_idx].detach().clone(), p[s:f]), dim=0)
                else:
                    K = K[s:f]
                    A = A[s:f]
                    p = p[s:f]
                ...
                all_prompts = [
                    list(range(start * pt, (start + 1) * pt))
                    for start in self.known_tasks
                ]
                all_prompts_idx = [
                    idx for old_prompt in all_prompts for idx in old_prompt
                ]
                K = K[all_prompts_idx]
                A = A[all_prompts_idx]
                p = p[all_prompts_idx]
...
class EPrompt(nn.Module):
    ...
    def forward(self, x_embed, prompt_mask=None, cls_features=None):
        ...
        if self.prompt_pool:
            ...
            out["prompt_idx"] = idx
            if self.use_prefix_tune_for_e_prompt:
                batched_prompt_raw = self.prompt[
                    :, :, idx
                ]  # num_layers, B, top_k, length, C
                (
                    num_layers,
                    dual,
                    batch_size,
            ...
            if self.use_prefix_tune_for_e_prompt:
                ...
                if self.same_key_value:
                    ...
                    if self.prompt_init == "zero":
                        ...
                        self.prompt = nn.Parameter(
                            torch.randn(prompt_pool_shape)
                        )  # num_layers, 2, length, num_heads, embed_dim // num_heads
                        nn.init.uniform_(self.prompt, -1, 1)
                batched_prompt = self.prompt.unsqueeze(0).expand(
                    -1, x_embed.shape[0], -1, -1, -1
                )
            else:
